...
object RoomsV2Validation {
    ...
    const val MAX_MESSAGE_ID_CHARS = 120
    ...
    fun validateClientMessageId(value: String): String? = validateFirebaseKey(
        value = value,
        maxChars = MAX_CLIENT_MESSAGE_ID_CHARS,
    )

    fun validateMessageId(value: String): String? = validateFirebaseKey(
        value = value,
        maxChars = MAX_MESSAGE_ID_CHARS,
    )

    private fun validateFirebaseKey(value: String, maxChars: Int): String? = when {
        value.isBlank() || value.length > maxChars -> "Identificador de mensaje inválido"
