...
class CreateRoomV2UseCase @Inject constructor(
...
class JoinRoomV2UseCase @Inject constructor(
    private val repository: RoomsV2Repository,
) {
    suspend operator fun invoke(request: JoinRoomV2Request): ZibeResult<RoomV2Membership> {
        ...
        if (request.mode == RoomV2IdentityMode.ANONYMOUS) {
            ...
            return repository.joinRoom(
                request.copy(alias = RoomsV2Validation.collapseSpaces(alias)),
            )
        }
...
class SendRoomV2TextUseCase @Inject constructor(
    private val repository: RoomsV2ChatRepository,
) {
    suspend operator fun invoke(
        thread: RoomV2Thread,
        text: String,
        ...
        RoomsV2Validation.validateClientMessageId(clientMessageId)?.let { return invalidInput() }
        if (replyToMessageId != null) {
            RoomsV2Validation.validateMessageId(replyToMessageId)?.let { return invalidInput() }
        }
        return repository.sendText(
            thread = thread,
            text = text.trim(),
...
private fun invalidInput() = ZibeResult.Failure(
