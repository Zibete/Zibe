...
`create_room_v2`, `join_room_v2`, `leave_room_v2` y `send_room_v2_text` son callables autenticadas. Las proyecciones relacionadas se escriben mediante un único `update()` multipath de RTDB cuando no hace falta leer-modificar-escribir. Firebase documenta esas actualizaciones como atómicas dentro de Realtime Database.
...
- `functions/legacy_main.py` conserva byte por byte el entrypoint previo; `functions/main.py` solo expone ese módulo y agrega las callables V2.
